...
static csiebox_protocol_status sync_file_meta(csiebox_client* client, char* path, int fd);
static void sync_file_data(csiebox_client* client, char* path, int fd);
...
BUSY1:
	...
	if (recv_message(client->conn_fd, &header, sizeof(header))) {
		if (header.res.magic == CSIEBOX_PROTOCOL_MAGIC_RES && header.res.status == CSIEBOX_PROTOCOL_STATUS_BUSY){
...
static void sync_file(csiebox_client* client, char* path) {
	...
	struct flock lock = {F_RDLCK, SEEK_SET, 0, 0, 0}, rtest;
	lock.l_pid = getpid();
	rtest = lock;
	...
	if (fd < 0) {
		...
		if (fcntl(fd, F_SETLKW, &lock) < 0) {
			fprintf(stderr, "fail to lock file\n");
		}
		status = sync_file_meta(client, path, fd);
		if (status == CSIEBOX_PROTOCOL_STATUS_MORE) {
			sync_file_data(client, path, fd);
		}
		lock.l_type = F_UNLCK;
...
static csiebox_protocol_status sync_file_meta(csiebox_client* client, char* path, int fd) {
	...
	if ((meta.message.body.stat.st_mode & S_IFMT) == S_IFDIR) {
		...
		md5_file(path, meta.message.body.hash, fd);
	}
	int busy;
	csiebox_protocol_header header;
	do {
...
static void sync_file_data(
		csiebox_client* client, char* path, int fd) {
