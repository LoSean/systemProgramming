...
static void handle_request(csiebox_server* server, int conn_fd, fd_set *master, int busy);
...
/*					workload = 0;
					for(int j = 0; j < demandThread; j++){
						fprintf(stderr, "data.worker[%d] = %d, ", j, data.worker[j]);
						if (data.worker[j] == i){
							run = 0;
							break;
						}
...
*/
					...
					if (run){
						...
						if (data.workload >= demandThread){
							...
							handle_request(server, i, &master, 1);
						}
						else{
							data.job++;
							data.worker[i] = 1;
...
static void *threadRun(void *data){
	...
	while(1){
		...
		tmp->job--;
		pthread_mutex_unlock(&lock);
		handle_request(server, conn_fd, master, 0);	
...
static void sync_file(csiebox_server* server, int conn_fd, csiebox_protocol_meta* meta, int busy) {
	...
	struct flock lock = {F_WRLCK, SEEK_SET, 0, 0, 0}, lockTmp;
	lock.l_pid = getpid();
	lockTmp = lock;
//	fcntl();
