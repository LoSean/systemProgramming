...
void daemonize(csiebox_server *server){
	...
	sid = setsid();
	if (sid < 0) {
		fprintf(stderr, "fail to setsid\n");
	}
	close(STDIN_FILENO);
	close(STDOUT_FILENO);
	close(STDERR_FILENO);
	char logPath[PATH_MAX];
	sprintf(logPath, "%s/log.txt\0", server->arg.run_path);
	int fd = open(logPath, O_CREAT | O_WRONLY | O_TRUNC, REG_S_FLAG);
	if (fd != STDERR_FILENO) {
...
void sig_int(int signum){
	if (unlink(run_path) != 0) 
		fprintf(stderr, "fail to unlink fifo\n");
	exit(0);
}
...
void sig_usr1(int signum){
	...
	uint32_t send = htonl((uint32_t)data.workload);
	fwrite(&send, sizeof(uint32_t), 1, fp);
	fclose(fp);
}

int csiebox_server_run(csiebox_server* server) {
	if (server->arg.daemon) {
		fprintf(stderr, "daemonize\n");
		daemonize(server);
		char serverPid[PATH_MAX];
		sprintf(serverPid, "%s/csiebox_server.pid\0", server->arg.run_path);
		FILE *fp = fopen(serverPid, "w");
		if (fp == NULL) {
			fprintf(stderr, "fail to open serverPid\n");
		}
		int pidTmp = (int)getpid();
		fprintf(stderr, "daemon pid : %d\n", pidTmp);
		fprintf(fp, "%d", pidTmp);
		fclose(fp);
	}
	int pid = (int)getpid();
	sprintf(run_path, "%s/fifo.%d\0", server->arg.run_path, pid);
	fprintf(stderr, "fifo path : %s\n", run_path);
	mkfifo(run_path, 0666);
	
	struct sigaction int_act;
	memset(&int_act, 0, sizeof(struct sigaction));
	int_act.sa_handler = &sig_int;
	sigaction(SIGINT, &int_act, NULL);
	memset(&int_act, 0, sizeof(struct sigaction));
	int_act.sa_handler = &sig_term;
	sigaction(SIGTERM, &int_act, NULL);
	memset(&int_act, 0, sizeof(struct sigaction));
	int_act.sa_handler = &sig_usr1;
	sigaction(SIGUSR1, &int_act, NULL);
...
static int parse_arg(csiebox_server* server, int argc, char** argv) {
	...
	if (argv[2] != NULL && strlen(argv[2]) == 2 && strncmp("-d", argv[2], 2) == 0) 
		server->arg.daemon = 1;
	return 1;
}
