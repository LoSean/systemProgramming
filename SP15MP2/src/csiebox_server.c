...
static void syncRm(csiebox_server* server, int conn_fd, csiebox_protocol_rm* rm, int fdmax);
...
static void reClientRm(char *pathTmp, csiebox_server *server, int conn_fd, int fdmax);
		...
		switch (header.req.op) {
			...
			case CSIEBOX_PROTOCOL_OP_RM:
				...
				if (complete_message_with_header(conn_fd, &header, &rm)) {
					syncRm(server, conn_fd, &rm, fdmax);
					//====================
					//        TODO
					//====================
				}
...
static void syncRm(csiebox_server* server, int conn_fd, csiebox_protocol_rm* rm, int fdmax){
	...
	send_message(conn_fd, &header, sizeof(header));
//static int reSyncRm(char *pathTmp, csiebox_server *server, int conn_fd){
	reClientRm(pathTmp, server, conn_fd, fdmax);
	return;
}
